...
gilead_DTM <- DocumentTermMatrix(gilead_corpus, control = list(stemming = TRUE, stopwords = TRUE,
                                                      minWordLength = 2, removeNumbers = TRUE, 
                                                      removePunctuation = TRUE))
inspect(gilead_DTM[1:20,1:20])
str(gilead_DTM )
#remove sparse terms
gilead_DTM=removeSparseTerms(gilead_DTM , 0.990)
...
gileadreduced_DTM=gilead_DTM[,term_tfidf >= .022781]
summary(slam::col_sums(gileadreduced_DTM))

#inspect popular words
findFreqTerms(gileadreduced_DTM, 1000)
findFreqTerms(gilead_DTM, 3000) #this might be better to use since
...
system.time(DTM_model=topicmodels::LDA(gilead_DTM,k, 
